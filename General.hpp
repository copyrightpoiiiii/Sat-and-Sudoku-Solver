...
private:
	...
	void inc_capacity () {
		...
		Listsize <<= 1;
		if (elem != NULL)elem = (T *) realloc (elem, Listsize * sizeof (T));
		else elem = (T *) malloc ((Listsize * sizeof (T)));
	}
...
public:
	myVector (void) : elem (NULL), num (0), Listsize (0) {
	}
	...
	int find (const T val) {
		...
		if (elem[l] == val)return l;
		if (elem[r] == val)return r;
		return -1;
	}
	...
	void qsort (int l, int r, LessThan lt) {
...
public:
	...
	bool heapProperty (int i) {
		return (size_t) i >= heap.size ()
		       || ((parent (i) == 0 || !comp (heap[i], heap[parent (i)])) && heapProperty (left (i)) &&
